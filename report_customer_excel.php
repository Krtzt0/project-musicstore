...
$objPHPExcel->setActiveSheetIndex(0)
            ->setCellValue('A1', 'รายงานข้อมูลสินค้า')
            ->setCellValue('A3', 'รหัสลูกค้า')
            ->setCellValue('B3', 'ชื่อลูกค้า')
			->setCellValue('C3', 'อีเมล')
            ->setCellValue('D3', 'เบอร์โทร')
            ->setCellValue('E3', 'วันเกิด');

// กำหนดรูปแบบข้อความลงไปใน cell 
$objPHPExcel->getActiveSheet()->getStyle('A3:E3')->applyFromArray($datastyletitle);
...
$objPHPExcel->getActiveSheet()->getColumnDimension('E')->setWidth(30); // กำหนดความกว้างของแถว
...
{
	$objPHPExcel->getActiveSheet()->setCellValue('A' . $i, $objResult["Customer_id"]);
	$objPHPExcel->getActiveSheet()->setCellValue('B' . $i, $objResult["Customer_name"]);
	$objPHPExcel->getActiveSheet()->setCellValue('C' . $i, $objResult["Customer_email"]);
    $objPHPExcel->getActiveSheet()->setCellValue('D' . $i, $objResult["Customer_phone"]);
    $objPHPExcel->getActiveSheet()->setCellValue('E' . $i, $objResult["Customer_birth"]);
   $objPHPExcel->getActiveSheet()->getStyle('A'. $i.':E'.$i)->applyFromArray($datastyle);
	$i++;
}

...
$objPHPExcel->setActiveSheetIndex(0)
            ->setCellValue('A1', 'รายงานข้อมูลรายละเอียดประเภทสินค้า')
            ->setCellValue('A3', 'รหัสประเภทสินค้า')
            ->setCellValue('B3', 'ชื่อลูกค้า');
			

// กำหนดรูปแบบข้อความลงไปใน cell 
$objPHPExcel->getActiveSheet()->getStyle('A3:B3')->applyFromArray($datastyletitle);
...
{
	$objPHPExcel->getActiveSheet()->setCellValue('A' . $i, $objResult["Type_id"]);
	$objPHPExcel->getActiveSheet()->setCellValue('B' . $i, $objResult["Type_name"]);
   $objPHPExcel->getActiveSheet()->getStyle('A'. $i.':B'.$i)->applyFromArray($datastyle);
	$i++;
}
